...
- user is a recipient/creator or admin before mutating an annual event.

## Workers AI

Workers AI task creation is optional and disabled by default in the public template.

Treat model output as untrusted input:

- validate the requested action and supported task type;
- accept only temporary member references generated by the application;
- validate calendar dates, window order, time, title, and assignees;
- require explicit user confirmation before writing a task;
- never place tokens, Telegram IDs, D1 IDs, or session secrets in prompts;
- do not follow instructions embedded in member names or aliases.

If the model or quota is unavailable, the bot should fall back to normal menu/guided behavior rather than bypass validation.
...
D1 contains personal and household data such as Telegram user IDs, names, tasks, annual events, recipients, history, notification logs, and audit logs.
